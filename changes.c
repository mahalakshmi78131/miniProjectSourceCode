...
//Increase the first name buffer from 10 to 15 characters to accommodate longer names.

//File Initialization  initializeFile()
//This is a **brand new function** not present in the original. It checks if credit.data exists; if not, it creates the file and pre-fills it with 100 blank records. The original required the file to already exist and would simply exit if it did not.
...
//deposit() Dedicated function that adds a positive amount to a specific accounts balance with validation.
